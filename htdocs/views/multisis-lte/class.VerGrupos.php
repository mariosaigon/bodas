...
return  $newDate;
 ...
 function contarGrupos($dms)
	 {
	 	$res=true;
		$db = $dms->getDB();
		$consultar = "SELECT COUNT(*) FROM app_grupo;";
		//echo "Consultar: ".$consultar;
		$res1 = $db->getResultArray($consultar);
		return $res1[0]['COUNT(*)'];
	 }

class SeedDMS_View_VerGrupos extends SeedDMS_Bootstrap_Style 
{
 /**
 ...
 **/
	...
	{ /* {{{ */
		...
		$this->htmlStartPage("Lista de grupos", "skin-blue sidebar-mini  sidebar-collapse");
		$this->containerStart();
		$this->mainHeader();
		$this->mainSideBar();
 ...
 $this->startBoxPrimary("Lista de grupos");
$this->contentContainerStart();
//////INICIO MI CODIGO
?>
 <div class="box">
            <div class="box-header">
              <h3 class="box-title">Ver grupos de personas consideradas en la base de datos de la ENAFOP</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body no-padding">
              <table id="tablaEventos" class="table table-hover table-striped table-condensed">
               ...
               <tbody>
                	<?php
                	$numGrupos=contarGrupos($dms);
                	//////////////// DIBUJO TABLA
                	$consultar = "SELECT * FROM app_grupo;";
					//echo "Consultar: ".$consultar;
				  	$res1 = $db->getResultArray($consultar);
                	for($cont=0;$cont<$numGrupos;$cont++)
                	{
                		echo ' <tr>';
                		//1. nombre
                		 echo "<td>".$res1[$cont]['nombre']."</td>";
          
                		  // 2. descripcion
                		  echo "<td>".$res1[$cont]['descripcion']."</td>";
                       // 3. enlace para editar
                      $idEvento=$res1[$cont]['id'];
                       echo "<td><a href=\"/out/out.EditarGrupo.php?id=".$idEvento."\">Editar grupo</a></td>";
      

                        // 4. Ver miembros
                       echo "<td><a href=\"/out/out.VerMiembrosGrupo.php?id=".$idEvento."\">Ver miembros</a></td>";
